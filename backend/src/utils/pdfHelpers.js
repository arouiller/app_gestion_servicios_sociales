    ...
    .map(fila => {
      ...
      const celdasHTML = fila.celdas
        .map(celda => {
          ...
          return `<td style="width: ${anchoPorcentaje}%; border: ${borderStyle}; padding: ${paddingPx}px; vertical-align: top; overflow: hidden; white-space: pre; max-height: 100%; box-sizing: border-box;">${celda.contenido || ''}</td>`;
        })
        .join('');
