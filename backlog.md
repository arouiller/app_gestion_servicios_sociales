...
|----|-----------|--------|-------------|-------------------|----|
| BACKLOG-043 | 🔴 Alta | 📋 Registrado | Nueva entidad Zona independiente con CRUD | Nueva tabla Zona (código: 2 dígitos, nombre: string). Necesaria para soporte de geografía multinivel: Provincia → Localidad → Zona. CRUD completo en interfaz de gestiones. Requiere migración, modelos, controladores, servicios, componente UI. | migrations/v2.0.x, Zona.js model, zonaController.js, zonasService.js, GestionZonas.jsx |
...
| BACKLOG-041 | 🔴 Alta | 📋 Registrado | Renombrar entidad Zona a Localidad en toda la aplicación | Cambio terminológico: Provincia (1) → (N) Localidades. Nueva entidad separada llamada Zona. Afecta BD (tablas, constraints), backend (modelos, rutas, respuestas), frontend (labels, mensajes, componentes). Requiere migración de datos y refactor en cascada. | Múltiples (modelos, controllers, servicios, componentes UI) |
