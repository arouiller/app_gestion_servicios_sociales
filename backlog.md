...
| BACKLOG-078 | 🟡 Media | 📋 Registrado | Agregar parámetro "valor_cuota_social" y renombrar sección de configuración | Crear nuevo parámetro del sistema `valor_cuota_social` que acepte valores numéricos con decimales (coma). Agregarlo en ConfiguracionNotificaciones bajo la sección "Redondeo de cuotas". Renombrar sección "Redondeo de cuotas" a "Parametros de configuracion". Requiere: migración BD para agregar parámetro, actualización de seed, cambios en ConfiguracionNotificaciones.jsx, actualización de validación y almacenamiento del parámetro. | migrations/2.0.28, SystemConfig.js, ConfiguracionNotificaciones.jsx, configService.js |
| BACKLOG-079 | 🔴 Alta | 🚀 Desarrollado | Registrar 3 valores en tabla Recibo: cuota_social, arancel_por_servicio, valor_cuota | Implementado: (1) Migración 2.0.30 agrega cuota_social y arancel_por_servicio a tabla recibos. (2) recibosController calcula arancel_por_servicio = valor_cuota - valor_cuota_social (obtenido de ConfiguracionApp). (3) Recibo.js model con ambos atributos DECIMAL(10,2). (4) ResumenDesglose component muestra tabla con breakdown de cuotas, alerta si arancel es negativo. (5) ReciboDetalleModal integra ResumenDesglose. Validación invariante cuota_social + arancel_por_servicio ≈ valor_cuota (tolerancia 0.01). Commits: múltiples, culminando en e519e71 (fix configuracion_app query). | migrations/2.0.30, Recibo.js, recibosController.js, ResumenDesglose.jsx, ReciboDetalleModal.jsx |
| BACKLOG-080 | 🔴 Alta | 📋 Registrado | Impresión de recibos con desglose y múltiples por página | Generar PDF de recibos con: (1) campos de desglose (cuota social, arancel por servicio, valor total cuota); (2) capacidad de imprimir hasta 2 recibos diferentes por página (no duplicados, recibos consecutivos). Template debe ser flexible para adaptarse a diferentes cantidades de recibos. | GenerarRecibosModal.jsx, recibosService.js, recibosController.js, nuevos templates PDF |
